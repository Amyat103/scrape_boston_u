...
def test_cascs():
    ...
    major = "CASCS"
    courses = scraper.get_courses_from_major(major)
    all_course_data = []
    max_courses = 3
    
    for course in courses['courses']:
        comp_details = scraper.get_complementary_details(course['crse_id'], course['effdt'], major, course['catalog_nbr'].strip(), course['typ_offr'])
        print("Received complementary details:", comp_details) 

        course_data = {'complementary_details': comp_details}
        term = get_term_from_offerings(comp_details)
        print(f"Term extracted: {term}") 
